...
void clean_objs(){
  ...
  delete gROOT->FindObject("angles");
  delete gROOT->FindObject("type");
  delete gROOT->FindObject("impulse");
  delete gROOT->FindObject("t_impulse");
  delete gROOT->FindObject("energy");
  ...
  delete gROOT->FindObject("canva");
  delete gROOT->FindObject("canva2");
}
...
void particle_sim() {
  ...
  TCanvas* canva = new TCanvas("canva", "histo_tot", 100, 100, 1500, 700);
  canva->Divide(2, 5);

  Int_t canva_idx = 1;
  for (; canva_idx <= 9; ++canva_idx) {
    canva->cd(canva_idx);
    list->At(canva_idx-1)->Draw();
  }

  TCanvas* canva_2 = new TCanvas("canva2", "2d_histo", 1600, 100, 1500, 700);
  list->At(canva_idx-1)->Draw("lego2");
